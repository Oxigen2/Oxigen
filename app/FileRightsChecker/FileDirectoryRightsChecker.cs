...
namespace OxigenIIAdvertising.FileRights
{
    ...
    public static class FileDirectoryRightsChecker
    {
        ...
        {
            string directoryName = directoryPath + "\\" + Guid.NewGuid();
        ...
        public static bool IsSingleFileReadableWritable(string filePath)
        {
            ...
            try
            {
                fsr = GetFilePermissions(username, filePath, currentUsersGroups);
            }
            catch
            {
                return false;
        ...
        private static FileSystemRights GetFilePermissions(string user, string path,
                                                           IdentityReferenceCollection currentUsersGroups)
        {
            if (!File.Exists(path))
                return 0;
            ...
            AuthorizationRuleCollection fsRules = fileSecurity.GetAccessRules(true, true, typeof (NTAccount));
        ...
        private static bool IsReadableWritable(FileSystemRights rightsFound)
        {
            ...
            FileSystemRights writeAccess = FileSystemRights.WriteData | FileSystemRights.ReadData |
        ...
        {
            string fileName = directoryPath + "\\" + Guid.NewGuid();
