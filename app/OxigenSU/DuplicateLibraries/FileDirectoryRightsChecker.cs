...
namespace OxigenSU
{
  public static class FileDirectoryRightsChecker
  {
    ...
    /// <summary>
    /// Checks if all files in a directory are readable/writable for the current user. This method
    /// does not recursively searches any sub directories that may exist
    /// </summary>
    /// <param name="directory">Path to check the files in</param>
    /// <returns>false if there is at least one file which doesn't fulfill the above rights, true otherwise</returns>
    public static bool AreFilesReadableWritableRootOnly(string directory)
    {
      WindowsIdentity currentIdentity = WindowsIdentity.GetCurrent();
      string username = currentIdentity.Name;
      IdentityReferenceCollection currentUsersGroups = currentIdentity.Groups.Translate(typeof(NTAccount));
